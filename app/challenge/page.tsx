    ...
    <div className="pb-24 space-y-6">
      ...
      {todayDone ? (
        ...
        <div className="card p-4 space-y-3">
          ...
          <div className="flex items-center justify-between">
            <span className="text-xs text-gray-500">익명으로 인증</span>
            <button
          ...
          >
            {submitting ? "인증 중..." : `${isAnonymous ? "익명으로 " : ""}인증하기`}
          </button>
        </div>
      )}
      ...
      <section>
        ...
        <div className="space-y-2">
            ...
            <div key={c.id} className="card px-4 py-3">
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs font-medium text-gray-600">
                  {c.is_anonymous ? "익명" : (c.user?.nickname ?? "익명")}
                </span>
          ...
          {challenges.length === 0 && (
            <p className="text-center text-gray-400 text-sm py-8">아직 인증이 없어요. 첫 번째로 인증해 보세요!</p>
          )}
        </div>
      </section>
    </div>
